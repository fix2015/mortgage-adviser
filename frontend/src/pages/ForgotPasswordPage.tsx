...
export function ForgotPasswordPage() {
  ...
  const onSubmit = async (data: FormData) => { setError(""); setIsLoading(true); try { await forgotPassword(data.email); setSuccess(true); } catch (err: unknown) { let m = "Something went wrong."; if (err && typeof err === "object" && "response" in err) { const ae = err as { response?: { data?: { detail?: string } } }; m = ae.response?.data?.detail || m; } setError(m); } finally { setIsLoading(false); } };
  return (
    <div className="flex min-h-screen items-center justify-center bg-ds-bg-primary px-4">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative w-full max-w-md">
        <div className="glass-strong rounded-2xl p-8 shadow-2xl">
          ...
          {success ? (<div className="text-center"><div className="mb-6 rounded-xl border border-ds-feedback-success/30 bg-ds-feedback-success/10 px-4 py-3 text-sm text-ds-feedback-success">Check your email for reset instructions.</div><Link to="/login" className="text-ds-text-accent hover:underline font-medium text-sm">Back to Sign In</Link></div>) : (<><form onSubmit={handleSubmit(onSubmit)} className="space-y-5"><Input label="Email" type="email" placeholder="you@email.co.uk" leftIcon={<Mail className="h-4 w-4" />} error={errors.email?.message} {...register("email")} /><Button type="submit" variant="glow" size="lg" className="w-full" isLoading={isLoading}>Send Reset Link</Button></form><p className="mt-6 text-center text-sm text-ds-text-secondary">Remember your password?{" "}<Link to="/login" className="text-ds-text-accent hover:underline font-medium">Sign In</Link></p></>)}
        </div>
      </motion.div>
    </div>
  );
